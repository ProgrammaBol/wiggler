import pygame
from engine.sprites import MovingSprite, StaticSprite
from engine.animations import Animation
from engine.exceptions import AnimationEnded
...
class Bullet(MovingSprite):
    ...
    def __init__(self,
        ...
        self.animations["explode"] = self.game_context.animations.get_animation(
            self,
            "bulletexplode",
            0
        )
...
class Laser(StaticSprite):
    ...
    def __init__(self,
        ...
        self.costumes["default"] = pygame.Surface(
            (game_context.resolution[0], game_context.resolution[1])
        )
    ...
    def prepare_animation(self, hit=False):
        ...
        for radius in radiuses:
            ...
            pygame.draw.circle(
                image,
                (255, 255, 255),
                (self.parent.rect.centerx, self.parent.rect.centery),
                radius
            )
            self.animations['ray'].append_frame(
                image,
    ...
    def nada(self):
        ...
        for radius in radiuses:
            ...
            pygame.draw.circle(
                image,
                (255, 255, 255),
                (self.ray_sourcex, self.parent.rect.centery),
                radius
            )
        for width in widths:
            image = self.costumes['default'].copy()
